...
wstring
PerestanCipher::CoderPerestanCipher(PerestanCipher w,wstring& s)
{
 wstring code;
...
wstring
PerestanCipher::DecoderPerestanCipher(PerestanCipher w, wstring& s)
{
 s=getValidOpenText(s);
 w=getValidKey(w.k,s);
 int h;
...
inline std::wstring
PerestanCipher::getValidOpenText(const std::wstring & s)
{
 std::wstring tmp;
 for (auto c:s) {
 if (isalpha(c)) {
...
inline std::wstring
PerestanCipher::getValidCipherText(const std::wstring &s)
{
 std::wstring tmp;
 for (auto c:s) {
 if (isalpha(c)) {
